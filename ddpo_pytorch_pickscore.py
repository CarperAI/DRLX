...
def parse_args():
    ...
    args.add_argument("--beta_1", type=float, default=0.9)
    args.add_argument("--beta_2", type=float, default=0.999)
...
@torch.no_grad()
def calc_score(prompts, images, aesthetic_processor, aesthetic_model, device):
    aesthetic_model.to(device)

    # preprocess
    image_inputs = aesthetic_processor(images=images, padding=True, truncation=True, max_length=77, return_tensors="pt").to(device)
    text_inputs = aesthetic_processor(text=prompts, padding=True, truncation=True, max_length=77, return_tensors="pt").to(device)

    image_embs = aesthetic_model.get_image_features(**image_inputs)
    image_embs = image_embs / torch.norm(image_embs, dim=-1, keepdim=True)

    text_embs = aesthetic_model.get_text_features(**text_inputs)
    text_embs = text_embs / torch.norm(text_embs, dim=-1, keepdim=True)


    scores = aesthetic_model.logit_scale.exp() * torch.einsum("bd,bd->b", text_embs, image_embs)
...
def imagenet_animal_prompts():
    animal = random.choice(imagenet_classes[150:175])
    prompts = f'{animal}'
    return prompts

def hf_datasets_prompts(): return random.choice(hf_datasets_prompts.dataset['train'][hf_datasets_prompts.key])
...
def sd_sample(prompts, pipe, height, width, guidance_scale, num_inference_steps, eta, device):
    ...
    latents = torch.randn((len(prompts), unet.config.in_channels, height//8, width//8)).to(device)
...
if __name__ == '__main__':
    ...
    aesthetic_processor = AutoProcessor.from_pretrained(processor_name_or_path)
    aesthetic_model = AutoModel.from_pretrained(model_pretrained_name_or_path).eval()
    ...
    reward_fn = partial(calc_score, aesthetic_processor=aesthetic_processor, aesthetic_model=aesthetic_model)
    ...
    def sample_and_calculate_rewards(prompts, pipe, image_size, cfg, num_timesteps, decoding_fn, reward_fn, device):
        ...
        rewards = reward_fn(prompts, imgs, device=device)
        return imgs, rewards, all_step_preds, log_probs

    
    hf_datasets_prompts.dataset = load_dataset('CarperAI/pickapic_v1_no_images_training_sfw')
    hf_datasets_prompts.key = 'caption'

    train_set = PromptDataset(hf_datasets_prompts, args.num_samples_per_epoch)
    ...
    optimizer = torch.optim.AdamW(pipe.unet.parameters(), lr=args.lr, weight_decay=args.weight_decay, betas=(args.beta_1, args.beta_2))
    ...
    for epoch in master_bar(range(args.num_epochs)):
        ...
        for inner_epoch in progress_bar(range(args.num_inner_epochs)):
            ...
            for i in progress_bar(range(len(all_step_preds_chunked))):
                ...
                if (i + 1) % args.grad_accum == 0 or (i+1) == len(all_step_preds_chunked): # grad accum
                    optimizer.step()
                    optimizer.zero_grad()
